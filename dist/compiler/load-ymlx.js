...
const cache = new Map();
async function transpileYmlx(filePath, visited = new Set()) {
...
export async function loadYmlx(filePath) {
    ...
    if (ymlxImports.length > 0) {
        ...
        for (const imp of ymlxImports) {
            ...
            const importCacheFile = path.join(cacheDir, `${path.basename(imp.resolvedPath, ".ymlx")}-import-${Date.now()}.mjs`);
    ...
    if (ymlxImports.length > 0) {
        for (const imp of ymlxImports) {
            ...
            if (importUrl) {
                ...
                jsCode = jsCode.replace(new RegExp(`import\\s+${imp.name}\\s+from\\s+["'][^"']+\\.ymlx["']`, "g"), `import ${imp.name} from "${importUrl}";`);
            }
        }
    }
    // Write to a temporary cache file (cacheDir already created above if needed)
    const cacheFile = path.join(cacheDir, `${path.basename(absPath, ".ymlx")}-${Date.now()}.mjs`);
